...
import { createSearch, type Result } from '@/lib/search'
...
function useAutocomplete({ onNavigate }: { onNavigate: () => void }) {
  ...
  let { doc, hrefFor } = useDocs()
  let searchRef = useRef<(query: string, opts?: { limit?: number }) => Array<Result>>(
    () => [],
  )
  useEffect(() => {
    searchRef.current = doc ? createSearch(doc, hrefFor) : () => []
  }, [doc, hrefFor])
        ...
        return [
          {
            ...
            getItems() {
              return searchRef.current(query, { limit: 5 })
            },
            getItemUrl({ item }) {
              return item.url
            },
